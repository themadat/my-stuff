...
At mobile widths (up to 700px), the inventory toolbar uses separate search, dropdown, quick-selection, and ownership/action rows. All/House/Me buttons toggle their details while filtering ownership, with session-local expansion choices. They default to condensed on mobile and expanded elsewhere. Expanded mobile cards wrap with the actions, preserving 44px targets. Room-only item sections precede the named spaces in the same room.
