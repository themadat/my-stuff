...
`);
  ...
  function locationHeadings(path, previous) {
    let changed=false;
    return path.map(function (name,index) { changed = changed || name !== previous?.[index]; return changed && name ? '<tr class="location-section-heading" data-location-level="'+index+'"><th colspan="5" id="'+esc(locationAnchor(path.slice(0,index+1)))+'" tabindex="-1">'+esc(name)+'</th></tr>' : ''; }).join('');
  }
  ...
  function renderList() {
    ...
    $("#inventoryList").innerHTML = '<div class="inventory-table-wrap"><table class="inventory-table"><caption class="visually-hidden">Inventory. Select a value to filter, or Edit to open an item.</caption><thead><tr><th>Object and Properties / Notes</th><th>Count</th><th>Value</th><th>' + (view === 'previous' ? 'Departure' : 'Obtained') + '</th><th>Actions</th></tr></thead><tbody>' + sections.map(function (section,sectionIndex) { return locationHeadings(section.path,sections[sectionIndex-1]?.path) + m.groupRows(section.items).map(function (members) {
