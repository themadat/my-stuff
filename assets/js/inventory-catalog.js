(function () {
  ...
  function render() {
    ...
    const chips = function (values) { return '<div class="catalog-chips">' + values.map(function (value) { return '<span>' + esc(value) + '</span>'; }).join("") + '</div>'; };
    ...
    const tags = catalog.tagGroups.map(function (g) { const values = g.tags.filter(function (tag) { return matches([g.name, tag]); }); return values.length ? '<section class="catalog-group"><h4>' + esc(g.name) + '</h4>' + chips(values) + '</section>' : ''; }).join("");
    const groups = catalog.propertyGroups.map(function (g) {
      ...
      return values.length ? '<section class="catalog-group"><h4>' + esc(g.name) + '</h4>' + values.map(function (p) { return '<div class="catalog-property"><strong>' + esc(p.name) + '</strong>' + (p.units.length ? ' <small>' + esc(p.units.join(" / ")) + '</small>' : '') + (p.values.length ? chips(p.values) : '') + '</div>'; }).join("") + '</section>' : '';
    }).join("");
  ...
  App.inventoryCatalog = { init: init, render: render, data: data };
})();
