...
const CACHE_NAME = "my-stuff-shell-0.0.1.26";
const ASSET_VERSION = "0.0.1.26";
...
self.addEventListener("activate", function (event) {
  event.waitUntil(Promise.all([
    caches.keys().then(function (keys) { return Promise.all(keys.filter(function (key) { return key.startsWith("my-stuff-shell-") && key !== CACHE_NAME; }).map(function (key) { return caches.delete(key); })); }),
    self.clients.claim()
  ]));
});
