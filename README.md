...
Use **Copies** to add up to 100 matching objects at once, optionally assigning each a different room. Blank room overrides use the location above; different rooms clear the inherited space and use the known parent zone. Price and value are **per copy**. Each copy is a separate record, so editing or archiving one leaves the others unchanged. **Add a Copy** on a saved item reuses its details without changing the original.
