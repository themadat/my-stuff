...
test('real controls upload only Notes, confirm cloud restore, retain settings, export without secrets', { timeout: 30000 }, async t => {
  ...
  await page.waitForFunction(() => window.LocalApp.sync.getInfo().state === 'upToDate');
  assert.deepEqual(h.writes[0], { syncFormat: 'local-first-app-data', syncVersion: 1, schemaVersion: 7, data: { inventory: { currency: 'USD', items: [] }, favoriteBrands: [], notes: 'Local ☁️\n<literal> Notes' } });
...
test('compact item form parses measurements and hovered row shortcuts respect typing', {timeout:30000}, async t => {
 ...
 assert.ok(notes.x>tags.x && Math.abs(notes.y-tags.y)<3);
 await page.locator('#addPropertyButton').click(); await page.locator('[data-property-name]').fill('Weight');
 await page.locator('[data-property-value]').fill('250g'); await page.locator('[data-property-value]').press('Tab');
 assert.equal(await page.locator('[data-property-value]').inputValue(),'250'); assert.equal(await page.locator('[data-property-unit]').inputValue(),'g');
 assert.equal(await page.locator('#itemProperties .property-equivalent').textContent(),'≈ 8.818 oz');
 const equivalent=await page.locator('#itemProperties .property-equivalent').boundingBox(), unit=await page.locator('[data-property-unit]').boundingBox();
 assert.ok(equivalent.x>unit.x+unit.width && equivalent.y>=unit.y && equivalent.y<unit.y+unit.height);
 await page.locator('#itemCopies').fill('2'); await page.locator('#itemCopies').press('Tab');
 assert.equal(await page.locator('[data-copy-location]').count(),2);
 const copy=await page.locator('[data-copy-location] > strong').first().boundingBox(), room=await page.locator('#copy0Room').boundingBox();
 assert.ok(Math.abs(copy.y+copy.height/2-room.y-room.height/2)<12);
 await page.screenshot({path:'/private/tmp/my-stuff-compact-editor-desktop.png'});
 await page.locator('#saveItemButton').click();
 const row=page.locator('tr').filter({has:page.locator('[data-edit-item]')}).first();
 assert.ok((await row.textContent()).includes('≈ 8.818 oz'));
 await row.hover(); await page.keyboard.press('e'); assert.equal(await page.locator('#itemDialog').isVisible(),true);
 await page.waitForFunction(()=>document.activeElement.id==='itemName');
 await page.locator('#itemName').press('a'); assert.equal(await page.locator('#archiveDialog').isVisible(),false);
 await page.locator('#itemName').fill('Metric Bottle'); await page.locator('#saveItemButton').click();
 await row.hover(); await page.keyboard.press('a'); assert.equal(await page.locator('#archiveDialog').isVisible(),true);
 await page.locator('[data-inv-close="archiveDialog"]').first().click();
...
test('quick categories combine selections and keep preset details visible', { timeout: 30000 }, async t => {
  ...
  const rows=await page.locator('#categoryTags .category-tag-option').evaluateAll(elements=>elements.map(el=>({top:el.getBoundingClientRect().top,bottom:el.getBoundingClientRect().bottom})));
  assert.ok(rows.slice(1).every((row,index)=>row.top>=rows[index].bottom));
  await page.locator('.category-tag-option:has([data-category-tag="Cables"]) .preset-description').click();
  assert.equal(await page.locator('[data-category-tag="Cables"]').isChecked(),true);
  await page.locator('.category-tag-option:has([data-category-tag="Phone"]) > span').click();
  assert.equal(await page.locator('[data-category-tag="Phone"]').isChecked(),true);
  await page.locator('.category-tag-option:has([data-category-tag="Phone"]) > span').click();
  assert.equal(await page.locator('[data-category-tag="Phone"]').isChecked(),false);
