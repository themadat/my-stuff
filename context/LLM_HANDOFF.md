...
Quantity controls now appear beside the pricing choices near the top of Add. New single entries default to overall price/value totals; explicit per-copy/piece mode and a live breakdown make interpretation visible. Total amounts distribute exactly in cents, with remainder cents assigned to the first copies. Existing edits and spreadsheet drafts initially use per-item amounts to avoid redividing saved/imported data. Increasing quantity works for new Have and Had entries. Existing untouched copy prices retain their differences; changing a per-item amount applies it to the edited copies.

Copies gain independent obtained-date overrides (unchecked Shared Date plus blank means unknown). Named pieces of one set have per-piece names and room/space/date controls, stored using the existing Set Piece property and copyGroup identity. Sets remain linked through edits/archive/return, and separate sets are not conflated. Piece values are allocated across records so inventory does not count the full purchase price multiple times. Bulk review preserves date/piece overrides and records saved amounts as per-item values. Fixed the old mobile copy-grid minimum width so all controls fit without horizontal scrolling.

Obtained and Gone Date text entry inserts ISO dashes for digit sequences starting with 20, otherwise month/day/year slashes. Invalid dates still show a validation error. Both direct Had and Archive forms offer Unknown Gone Date; unknown departure dates retain Had status and show unknown duration/annual cost. Replaced is a departure reason. Conveyed is an acquisition method with price $0 and date 2020-12-17 defaults in manual selection, Smart Add and mapped imports. Glassware automatically supplies Capacity, oz.

Verification: 129 non-browser checks pass. Eleven affected Chromium checks pass across individual/grouped copies, exact totals, separate dates/rooms, set identity, archived edits and invalid dates, bulk review/pause/resume, duplicate protection, location percentages/date picking, mobile layout and offline reload/edit. Reviewed desktop/mobile screenshots. Updated obsolete per-copy price, Sling spelling and space-progress expectations; stabilized editor test focus before typing. Version/query/cache/manifests/workflow aligned at .73; scripts and diff checks pass. Existing unrelated staged/manual edits preserved; bulk-entry.js has two necessary task edits on top of its existing working-tree content. Preview stopped before handoff; no commit or push performed.
...
All departure wording stays in Departure Notes. Rules suggest Broken for broken/chipped/leaking, Lost for missing/left-at/lost, Trashed for thrown-away/trashed, plus explicit sold/donated/given-away; ambiguous unknown lost/trash and replacement-only wording use Other. Suggestions remain editable. Added sample household brands and Appliance → Appliances alias. Corrected color inference to use the Color property vocabulary rather than Size.

Removed individual text caps in import preparation, item normalization and editor controls so names, seller/brand, notes, departure notes and property text survive review/save/reopen. Removed row-text and spreadsheet-column caps; whole-file/queue/batch limits and structural validation remain. Copy property overrides retain long values too. No sample records were added to user inventory; the 21 supplied rows are regression fixtures in tests/fixtures/household-had.tsv.

Verification: 121 non-browser checks and seven targeted Chromium checks pass. Tests cover all 21 supplied rows, optional category columns, invalid dates, ignored values/durations, long field round trips, review/pause/resume/save/reload, all rows remaining archived, current inventory totals, copy/skip/resume behavior, duplicate-save protection, escaped text, mobile and offline editing. Desktop/mobile screenshots reviewed; scripts, manifests, asset/version/cache/workflow surfaces and diffs pass. Corrected stale browser expectations for existing Save All Copies & Next, grouped skipping, explicit Float tags and Sling spelling. Existing unrelated staged/manual edits preserved. Preview stopped before handoff; no commit or push performed.
...
Favorite brands now enter the cloud payload as data.favoriteBrands, including explicit empty arrays for unfavoriting. Cloud schema 7 and the data-v3 hash prefix protect the field from older clients and migrate existing baselines. Reads accept prior schemas 5/6; downloads from those preserve local favorites. Explicit Merge Both Copies unions favorites. Device appearance and other preferences stay local, and Reset Preferences preserves these now-synced favorites. Update all browsers before syncing the new format. No real cloud writes were performed.

Verification: 117 non-browser checks pass, including two-browser simulated upload/download, favorite removal, older-cloud preservation, merge and invalid favorite validation. The browser regression passes alphabetical ordering, vertical geometry, full-row/description clicking, checkbox keyboard behavior and category OR filtering. Additional Chromium checks pass desktop/mobile layout, save/reload and offline use; screenshots reviewed. The supplementary browser check waits for the editor’s focus-restoration animation before testing keyboard entry. All release/query/cache/manifest/workflow surfaces align at .60. Existing unrelated staged/manual edits preserved. Preview stopped before handoff; no commit or push performed.

## Latest Update — Multiple Categories and Visible Presets (0.0.1.59)

Quick category cards now toggle independently. Any selected group or tag matches (OR), combined with existing search, location and ownership filters. Selected groups and groups with selected tags stay white; inactive cards gray out. A “Matching any” row provides individual remove buttons. The picker supports several tags from the same group, stays open after selection, retains keyboard focus, and closes on Escape or outside interaction. Selecting a tag replaces that group’s broad filter while retaining other selections. Group names stay stable instead of swapping to the last picked tag.

Preset property names and units appear directly under tag choices, editor property-set names and Settings catalog entries. Removed reliance on delayed native titles and floating property hints. Existing manual and staged changes were preserved. Version, query, cache, manifests and workflow surfaces advance to .59; Help and release notes describe the controls.

Verification: 115 non-browser checks pass. Targeted Chromium checks pass for desktop/mobile selection, white styling, repeated tag selections, keyboard navigation, visible presets, saving and offline reload; desktop and mobile screenshots reviewed. Added a browser regression covering OR filtering with three distinct tagged items, selection removal/clear, keyboard focus and visible presets. Preview server stopped before handoff. No commit or push performed.

## Latest Update — Household Tags and Bulk Archive Entry (0.0.1.58)

Around the House location rows put the Last Updated Date, count and value on the same line as the name (previously the date sat on its own row below). The name column truncates with an ellipsis instead of wrapping letter-by-letter when the row is tight, and the button's tooltip always carries the full location name so a truncated name stays discoverable at the default sidebar width. Inventory table rows tighten the space between an object's title and its Notes/tags line, and between the Date Obtained and Obtained-how filter buttons in the Obtained column.

Systems tags become Fire, Climate, Smart, Electrical, Plumbing, Security (removed HVAC and Fixture; Temperature renamed to Climate; Fan and Switch moved to Tech). Tech gains Lights, Fan and Switch. Lighting's LED renamed to Strip. Added Electrical, Plumbing and Security symbols from the user's supplied SVGs to the shared category-symbol registry. Category quick-filter cards desaturate and grey out once a sibling card is selected, using a `:has()` rule scoped to the toolbar's card row.

Settings → Inventory catalog: a Delete control beside each Custom Tag removes it from every current and archived item after a confirmation dialog. The catalog's Brands group now lists the full computed brand set (config brands, companies, saved Brand properties and favorites) rather than the static config list, and tag-group names are recognized as known vocabulary; both fixes keep Govee (a Brand) and Lighting (a tag-group name) out of Custom Tags.

Date Obtained and a location's Last Updated Date are free-text inputs instead of native date pickers. Clicking or typing into either field un-checks Unknown; Save accepts typed MM/DD/YYYY, MM/DD/YY, or YYYY-MM-DD text and shows "Enter a valid calendar date." if it cannot parse, rather than silently rejecting a typed value. Stuff I Had now exposes Bulk Add: the dialog retitles to "Bulk Add to Stuff I Had", prepared rows carry an archive draft (Gone Date, What Happened, Departure Notes parsed from the last date and a trailing reason/note in Smart Complete), and finishing or resuming a Had batch returns to Stuff I Had instead of Stuff I Have.

Fixed during this session's verification: the sidebar name/date/count/value layout above collapsed the name column to a single character at the default 230px sidebar width once date/count/value joined its row; switched the name's overflow handling from wrap-anywhere to ellipsis and gave it a 6ch floor. Updated three test expectations that were still asserting the pre-change vocabulary (total tag count 61→64, the Brands catalog group's dynamic length, and Lights no longer aliasing to Lighting), and fixed a pre-existing `tests/static.test.mjs` bug where `node --check` was invoked with an un-decoded `%20`-encoded file URL pathname, which fails whenever the checkout path contains a space (this iCloud checkout does).

Verification: all 115 non-browser checks pass (`sync`, `static`, `smart-entry`, `inventory-copies`, `bulk-import`, `bulk-entry`). Manually verified in Chromium at desktop and mobile widths: typed US/ISO Date Obtained values save and reload correctly and an invalid typed date shows the new error; Systems/Tech/Lighting reclassification and the three new icons render on saved items; category cards grey out siblings once one is selected; deleting a custom tag removes it from the saved item's categories; Govee and Lighting no longer appear under Custom Tags; and a pasted Bulk-Had row (the Den/Govee/Lights fixture) completes Start Review → Save with Gone Date/What Happened/Departure Notes pre-filled and lands back in Stuff I Had. No console errors observed. Added `.claude/launch.json` (a `python3 -m http.server 8765` config) to drive this verification; preview server stopped afterward. Version/cache/manifests/workflow were already aligned at 0.0.1.58 by the time this session picked up the work. No commit or push performed.
